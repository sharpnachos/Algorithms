...
def heuristic(node, G):
    #Finds minimum path for each node
    minimum = 2000
    for item in G[node].keys():
        tempy = G[node]
        if tempy[item] < minimum:
            temp = G[node]
            minimum = temp[item]
    return minimum
...
#Runs for each node
for node in RP.keys():
    if node != "Home": #if statement skips the node that its calculating the distance from
        distance = a_star(RP, 'Home', node, heuristic)
        distances[node] = distance #Stores distances along with the node in the dictionary
print(distances)
